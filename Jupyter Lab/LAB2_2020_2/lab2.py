...
#        data = pd.read_html("https://coinmarketcap.com/currencies/{}/historical-data/?start={}v&end={}".format(moneda,start,end))
        ...
        for i in (data_2): 
            ...
            for date,open_,high,close in zip(aux2,cuadro_extraido["Open"],
                                                 cuadro_extraido["High"],cuadro_extraido["Close"]):
                self.tabla.insert("","end",text=date,values=[open_,high,close])
        cuadro_extraido.drop(["Open","High"],axis=1,inplace=True)
        self.ax.cla()
        self.ax.set_title("Precio de Cierre ($) vs Fecha")
        self.ax.grid()
        cuadro_extraido.plot(ax=self.ax)
        self.graph.draw()
##
root = Tk()
