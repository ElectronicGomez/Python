...
class Chat_personal:
    def __init__(self,master):
        ...
        self.master.geometry("650x520+100+100")
        ...
        self.cboCOM1 = Combobox(self.frmperfil, values=self.port_list, textvariable=self.port_select1, state="readonly")
        ...
        self.text1.place(height=400, width=430, x=0, y=0)        


        
        
        self.frmescribe = ttk.LabelFrame(self.master, text="Enviar mensaje")
        self.frmescribe.place(height=100, width=600, x=250, y=420)
        
        self.lblText = tk.Label(self.frmescribe, text="Texto:")
        self.lblText.place(height=28, x=5, y=11)
        
        self.inText1 = Entry(self.frmescribe, width=40, textvariable=self.envia1)
        self.inText1.place(height=28, width=260, x=50, y=11)

        
        self.btnEnviar1 = Button(self.frmescribe, text="Send", width=12, command=self.envia_mensaje1, font='Verdana 8',  bg = "#4C7FBD",  fg = "white")
        self.btnEnviar1.place(height=28, width=70, x=320, y=11)
...
#            self.status.set("Connected")
    ...
    def Recibe_dato(self):
        ...
        if self.ser1.in_waiting > 0:
            ...
            self.text1.insert('end',str(self.fecha.get())+" - "+str(self.hora.get()) + "\n"+ self.chat1 + strSerial + '\n'+ '\n')
            self.text1.config(state="disabled")
        self.master.after(400, self.Response_loop)
